...
class BacktestEngine:
    ...
    def run(self, candles: pd.DataFrame, user_code: str, globals_whitelist: Dict[str, Any]) -> BacktestResult:
        ...
        for i, row in sig.iterrows():
            print(f"Processing bar idx={i}, timestamp={row['timestamp']}, price={row['close']}, high={row['high']}, low={row['low']}, open={row}")
            ...
            if pm.side and pm.total_qty() > 0:
                ...
                print(f"Bar idx={i}: checking TP/SL for {tranches_snapshot} tranches.")
                for t in tranches_snapshot:
                    ...
                    if tp_hit and sl_hit:
                        print(f"Bar idx={i}: tranche TP ve SL aynı anda vurdu, conflict_rule={self.cfg.conflict_rule}")
                        ...
                        print(f"Bar idx={i}: tranche {'TP' if tp_hit else 'SL'} vurdu.")
                    ...
                    print(f"Bar idx={i}: tranche TP/SL -> type={hit_type}, qty={qty}, price={exec_price}, pnl={pnl}, comm={commission}")
            ...
            if plan_for_i:
                print(f"Bar idx={i}: {len(plan_for_i)} planned orders.")
                print(f"Planned orders: {plan_for_i}")
            for po in plan_for_i:
                ...
                print(f"  Planned order için {len(candidates)} candidate fill bulundu.")
                print(f"  Candidate fills: {candidates}")
                ...
                print(f"  Side/order_type filtresi sonrası {len(candidates)} candidate fill kaldı.")
                ...
                if po.order_type == "limit":
                    ...
                    print(f"  Limit price filtresi sonrası {len(candidates)} candidate fill kaldı.")
                ...
                if pm.side and pm.side != incoming_side and pm.total_qty() > 0:
                    ...
                    trades.append(TradeEvent(
                        ...
                        fill_price=close_price, leverage=1.0,
                        used_pct_from=cur_used, used_pct_to=0.0,
                        qty=close_qty, notional=close_qty * close_price,
                        commission=commission_close, pnl_amount=pnl_close, pnl_pct=0.0,
                    ))
                    pm.close_all()
                    cur_used = 0.0
                    cur_dir = 0
                ...
                trades.append(TradeEvent(
                    ...
                    commission=commission, pnl_amount=0.0, pnl_pct=0.0,
                ))
                cur_used = next_used
            ...
            lev_series.append(cur_lev if pm.total_qty() > 0 else 1.0)
