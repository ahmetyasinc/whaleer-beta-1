...
from backend.trade_engine.process.save import aggregate_results_by_bot_id
...
"""
    ...
    with patch('backend.trade_engine.process.run_bot._get_last_price_1m', return_value=MOCK_LAST_PRICE) as mock_price, \
         patch('backend.trade_engine.process.run_bot._get_min_qty', return_value=MOCK_MIN_QTY) as mock_qty, \
         patch('backend.trade_engine.control.control_the_results.load_bot_context', return_value=MOCK_CONTEXT) as mock_ctx, \
         patch('backend.trade_engine.process.run_bot.log_info'), \
         patch('backend.trade_engine.process.run_bot.log_warning'), \
         patch('backend.trade_engine.process.run_bot.log_error'), \
         patch('backend.trade_engine.control.control_the_results.log_info'), \
         patch('backend.trade_engine.control.control_the_results.log_warning'):
