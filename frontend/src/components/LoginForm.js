...
export default function LoginForm({ locale }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Remember Me logic:
      // If rememberMe is checked, we keep default (Local storage -> persists across browser sessions).
      // If unchecked, we set it to Session storage (persists only for current tab/window).
      // Note: Supabase defaults to 'local', so we only need to switch to 'session' if unchecked.
      // However, changing persistence is async and affects the client global state.
            ...
            <div className="rounded-md shadow-sm">
              <div>
                ...
                <input
                  ...
                  id="email"
                  name="email"
                  placeholder={t("username")} // This now says "Email" or "E-Posta"
