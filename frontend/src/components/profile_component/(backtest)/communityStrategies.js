...
const CommunityStrategies = ({ onSelect }) => {
  const { t } = useTranslation("backtestCommunityStrategies");
  const { favorites, community } = useStrategyStore();
  const [searchTerm, setSearchTerm] = useState("");

  const filtered = (community || [])
    // önce sadece backtest izni olanları al
    .filter((s) => {
      const allowBacktesting =
        s?.release?.permissions?.allow_backtesting ??
        s?.approved_release?.permissions?.allow_backtesting ??
        false;
      return allowBacktesting === true;
    })
    // sonra isim aramasını uygula
    .filter((strategy) =>
