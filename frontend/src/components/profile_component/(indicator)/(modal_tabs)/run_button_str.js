  ...
  const fetchStrategyData = useCallback(async () => {
    try {
      ...
      const {
        ...
        prints = [],
        inputs = [],
        status = "success", // ✅ backend’den hata ya da success bilgisi
        message = "",
      } = response.data || {};
      ...
      insertOrReplaceLastSubStrategyData(
        ...
        prints,
        inputs,
        { status, message } // ✅ error/success bilgisi store’a gönderiliyor
      );
...
const fetchStrategyData = useCallback(async () => {
  try {
    ...
    console.log("== INSERT STRATEGY DATA ==");
    console.log({ strategyId, status, message });
