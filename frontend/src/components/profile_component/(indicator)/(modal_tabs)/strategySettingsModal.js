...
const StrategySettingsModal = ({ isOpen, onClose, strategyId, subId }) => {
  ...
  // Hydration ve Portal için mount kontrolü
  useEffect(() => {
    setMounted(true);
  ...
  if (!mounted || !isOpen || !strategyId || !subId) return null;
                ...
                {input.type === 'bool' ? (
                  <div className="flex items-center justify-between py-1">
                    ...
                    <button
                      ...
                      className={`relative w-11 h-6 rounded-full transition-colors duration-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-950 focus:ring-blue-600 ${formState[input.name] ? 'bg-blue-600' : 'bg-zinc-700'
                        }`}
                    >
                      <span
                        className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform duration-100 transform ${formState[input.name] ? 'translate-x-5' : 'translate-x-0'
                          }`}
                      />
                    </button>
                  </div>
