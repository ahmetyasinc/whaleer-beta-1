...
export const PublishStrategyModal = ({ isOpen, onClose, onPublish }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      setLoading(true);
      // onPublish Promise döndürebilir/döndürmeyebilir — ikisini de güvenli şekilde ele al
      await Promise.resolve(onPublish?.({ permissions, description }));
    } finally {
      // Başarılı/başarısız fark etmeksizin istenen davranış: formu sıfırla
      resetForm();
      onClose && onClose();
    }
  };
      ...
      <div className="bg-gradient-to-br from-slate-900/95 via-slate-800/95 to-slate-900/95 backdrop-blur-xl text-white rounded-2xl w-[850px] max-h-[90vh] shadow-2xl relative flex flex-col border border-slate-700/50 animate-in zoom-in-95 duration-300 overflow-hidden">
        ...
        <div className="relative p-3 pt-3 border-t border-slate-700/50 bg-slate-900/30">
          <div className="flex justify-end gap-3">
            ...
            >
              <span className="relative z-10">{loading ? 'Publishing…' : 'Publish'}</span>
          ...
          <div className="bg-gradient-to-br from-slate-800/95 to-slate-900/95 backdrop-blur-xl text-white p-6 rounded-2xl w-[450px] shadow-2xl relative border border-slate-700/50 animate-in zoom-in-95 duration-200">
            ...
            <div className="relative">
              ...
              <div className="space-y-4 text-slate-300 leading-relaxed">
                <p>From here, you can determine which features of your strategy are accessible to users.</p>
                <p>Each permission affects how your strategy will appear and what actions can be taken with it.</p>
                <div className="p-3 bg-blue-500/10 rounded-lg border border-blue-500/20">
                  <p className="text-blue-300 text-sm">💡 <strong>Tip:</strong> Granting more permissions can help your strategy reach a broader audience.</p>
                </div>
              </div>
