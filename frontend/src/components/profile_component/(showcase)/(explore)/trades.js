...
const Trades = () => {
  const { currentBotData: botData } = useBotDataStore();

  if (!botData) {
    return <div className="text-gray-400">İşlem verisi yükleniyor...</div>;
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            <BiTransfer className="w-4 h-4 text-gray-300 flex-shrink-0" />
            <h3 className="text-sm font-semibold text-white">Geçmiş İşlemler</h3>
            <span className="text-xs text-gray-400">({botData.trades.length} işlem)</span>
          </div>
          <div className="bg-gradient-to-r from-gray-950 to-zinc-900 rounded-xl p-3 sm:p-4">
        ...
        {/* Açık Pozisyonlar / Kar-Zarar */}
        <div>
          <div className="flex items-center gap-2 mb-3">
            <BiTransfer className="w-4 h-4 text-gray-300 flex-shrink-0 rotate-90" />
            <h3 className="text-sm font-semibold text-white">Açık Pozisyonlar</h3>
            <span className="text-xs text-gray-400">({botData.positions.length} pozisyon)</span>
          </div>
          <div className="bg-gradient-to-r from-gray-950 to-zinc-900 rounded-xl p-3 sm:p-4">
            <div className="space-y-2">
              {botData.positions.map((pos) => (
