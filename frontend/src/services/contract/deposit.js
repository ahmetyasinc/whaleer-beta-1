// src/app/services/contract/deposit.js
"use client";
...
/**
 * Vault kontratına deposit işlemi yapar.
 *
 * @param {Object} params
...
export async function depositToVault({
  ...
  console.group("🚀 Deposit Process Started");
  ...
  try {
    ...
    if (!numericAmount || isNaN(numericAmount) || numericAmount <= 0) {
      throw new Error("Geçersiz depozito miktarı");
    }
    ...
    const op = contract.call("deposit", botIdScVal, userScVal, amountScVal);
    ...
    const assembledBuilder = assembleTransaction(tx, sim); // TransactionBuilder
    tx = assembledBuilder.build(); // Transaction
    ...
    // --- 9. Gönderim ---
    console.log("🚀 Transaction ağa gönderiliyor...");
    ...
    if (sendResp.status === "ERROR" || sendResp.errorResult) {
      ...
      throw new Error("Transaction gönderimi başarısız oldu.");
    }

    console.log("✅ Gönderildi! Hash:", sendResp.hash);
    console.log(
      "⏳ On-chain final onayı frontend'te POLL edilmiyor (Bad union switch bugundan kaçınmak için)."
    );
    ...
    console.log("🎉 İşlem gönderildi! Local status:", txStatus);
    console.groupEnd();

    // Burada backend'e POST atıp DB'de depozito kaydını oluşturmak için
    // frontend'te bu return değerini kullanabilirsin.
    return {
      hash: sendResp.hash,
      status: txStatus,
    };
  } catch (error) {
    console.error("🚨 depositToVault CATCH:", error);
    console.groupEnd();
    throw error;
  }
}
