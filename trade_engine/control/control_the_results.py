...
from trade_engine.data.bot_features import load_bot_context
from trade_engine.log.log import log_info, log_warning, log_error
from trade_engine.log.telegram.telegram_service import notify_user_by_telegram
...
def control_the_results(user_id, bot_id, results, min_usd=10.0, ctx=None):
    ...
    print(f"--- [Bot {bot_id}] Current State ({bot_type.upper()}) ---")
    if bot_type == "spot":
        if not holdings:
            print("   (No Holdings)")
        else:
            for h in holdings:
                ...
                print(f"   Asset: {h.get('symbol')} | Amt: {val} | Pct: {pct}%")
    else:
        if not positions:
            ...
            for p in positions:
                ...
                print(f"   Pos: {sym} ({side}) | Amt: {amt} | Lev: {lev}x | Pct: {pct}%")
    print("--------------------------------------------------")
    ...
    if bot_type == "spot":
        for sym, tgt in targets.items():
            ...
            if delta_spot < 0:
                ...
                if ok:
                    ...
                    # Eğer tam kapatıyorsak amount bilgisini de ekleyelim
                    if cur["spot_pct"] <= 1e-9: # çok küçükse 0 varsay
                        act["amount"] = cur["spot_amount"]
        ...
        for sym, tgt in targets.items():
            ...
            if cur_pct > 0 and target_pct > 0 and (cur_lev is not None and target_lev is not None) and (abs(cur_lev) != abs(target_lev)):
                ...
                if cur_lev is not None: act["leverage"] = abs(cur_lev)
                # Tam kapatma (Lev değişimi için) -> amount kullan
                act["amount"] = abs(cur["long_amount"])
                if cur_lev is not None and abs(cur_lev) > 1:
                    act["amount"] *= float(abs(cur_lev))
            ...
            if delta_long < 0:
                ...
                # Eğer target 0 ise tam kapatmadır -> amount kullan
                if target_pct <= 0:
                     act["amount"] = abs(cur["long_amount"])
                     if cur_lev is not None and abs(cur_lev) > 1:
                        act["amount"] *= float(abs(cur_lev))
            ...
            if cur_pct > 0 and target_pct > 0 and (cur_lev is not None and target_lev is not None) and (abs(cur_lev) != abs(target_lev)):
                ...
                if cur_lev is not None: act["leverage"] = abs(cur_lev)
                # Tam kapatma (Lev değişimi için) -> amount kullan
                act["amount"] = abs(cur["short_amount"])
                if cur_lev is not None and abs(cur_lev) > 1:
                    act["amount"] *= float(abs(cur_lev))
