...
from trade_engine.order_engine.core.price_store import price_store, PriceTicker
...
async def main():
    ...
    # Servisi Başlat
    with patch("trade_engine.order_engine.data_access.repos.crud.get_api_credentials_by_bot_id", return_value=mock_creds_return) as mock_get_creds, \
         patch("trade_engine.order_engine.data_access.repos.crud.insert_bot_trade", new_callable=AsyncMock) as mock_insert, \
         patch("trade_engine.order_engine.data_access.repos.symbol_filters.SymbolFilterRepo.initialize", return_value=True):
